...
PRFplot <- function (matrix,respID,h=.09,N.FPts=101,VarBands=TRUE,VarBands.area=TRUE,alpha=.05) {
  matrix <- as.matrix(matrix);
  ...
  axis(1,at=seq(0,1,by=.2)); mtext(side=1,text="Item difficulty",line=2.5,col="black",cex=1.5,font=1);
  axis(2,at=seq(0,1,by=.2)); mtext(side=2,text="Probability correct answer",line=2.8,col="black",cex=1.5,font=1,las=3);
  ...
  mtext(side=3,text=paste("PRF (respID # ",respID,")",sep=""),line=.5,col="black",cex=1.5,font=2);
}
